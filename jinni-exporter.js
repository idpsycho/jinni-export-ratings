...
// wtf, neither $.trim, nor String.trim didnt work even though they are defined
...
{
	var arr = localStorage_getArray(id);
	arr.append( arrAppend );
	localStorage_set(id, arr);
}

function localStorage_del(id)
{
	delete localStorage[id];
}
function localStorage_set(id, x)
{
...
function localStorage_getArray(id)
{
	var x = localStorage_get(id);
...
function isLastPage()
{
	...
	var btnText = trim(btnNext.text()+'').toLowerCase();
...
function readyJinniRatings()
{
	...
	{
		arrNewRatings = extractRatings();
		// append new ratings
		for (var i=0; i < arrNewRatings.length; i++)
		{
			var newR = arrNewRatings[i];
			var orig = g_arrRatings.findByAttr('title', newR.title);

			if (orig)	orig.rating = newR.rating;
			else		g_arrRatings.push( newR );
		}
...
function extractRatings(html)
{
	...
	$('ul.ratingStrip > li').each(function(i, e)
	{
		e=$(e);
		// use full-length tooltip (page content is shortened for long names)
		var title = e.find('.title').parent().attr('title');
		var rating = e.find(".digitRate").text();
		var link = e.find("a:eq(0)").attr('href');
		title = trim(title);
		rating = parseInt(rating);

		arr.push( {title: title, rating: rating, link: link} );
	});
...
function extractImdbFromLink(title, link)
{
	$.get(link,
		(function(ttl, lnk) {
			...
			{
				extractImdbFromHtml(ttl, resp, lnk);
			}
		})(title+'', link+'')
	);
}

function extractImdbFromHtml(title, resp, link)
{
	resp		= (''+resp).replace(/<img/ig, '<imgx');
	var page	= $(resp);
	var imdb	= ''+page.find('#onTheWeb .imdb').parent().parent().find('a').attr('href');
	var title_	= trim(''+page.find('#contentTitle_heading span:eq(0)').text());
	var year	= ''+page.find('#contentTitle_heading span:eq(1)').text();
	year		= (year.match(/[\d]+/)||[])[0];
...
{
	var data = encodeURIComponent( ratingsInCSV() );
	window.location.href = 'data:text/csv,'+data;
}
...
function exportingToggle(action)
{
	...
	{
		var b = isExportingEnabled();
		var s = ' - <b>{0}</b> '.format( b?'ENABLED':'DISABLED' );
		if (b)
			return '<a href="#" onclick="return exportingToggle(0)">stop</a>'+s;
		else
...
function showInfo(msgAdd)
{
	...
	//msg += ' - <a href="#" onclick="return saveExported()">save as file</a>';

	var style = ' style="font-size: 24px; background: #A6AF6C; color: #fff; \
				font-weight: bold; padding: 10px 20px;"';
	var info = $('<div id="jinniExporter" '+style+'></div>');
	info.html(msg);

	var styleTA = 'style="height: 100px; width: 550px; font-size: 9px; display: block; white-space: pre-wrap;"';
	var exported = $('<textarea id="jinniExporterResults" '+styleTA+'>');
	exported.text( ratingsInCSV() );
	info.append( exported );

	$('.ratingStrip:eq(0)').prepend( info );
}
...
/*
CLOSURE TEMPLATE:
$.get(url,					| (function(closured) { return
	function(resp)
	{
	}						| })(closured)
);
...
function define_helpers()
{
	...
	Array.prototype.findPosByAttr = function(name, value)
	{
		for (var i=0; i < this.length; i++)
		{
			if (this[i][name] == value)
